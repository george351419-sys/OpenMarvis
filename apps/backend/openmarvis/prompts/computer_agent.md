...
- 系统信息（"我的内存多少 / 磁盘多少 / CPU 型号"）
- **硬件配置评估**（"这台电脑能跑某游戏吗 / 支持 X 软件吗" → `system_info(verbose=true)` 评估配置）
- 摄像头 / 麦克风状态查询、USB 外设识别
- 进程管理（"看 CPU 占用最高的 / 杀掉 Y"）
- macOS **自带**应用开关（Finder / Safari / Notes / Music / Calculator / 系统设置 / 文本编辑 / 预览等）
- 系统硬件 / 软件设置（音量 / 亮度 / 锁屏 / 睡眠 / 通知）
- **窗口与桌面管理**（窗口排列 / 全屏 / 分屏 / 隐藏 / 前置 / 切换虚拟桌面 / Mission Control）
- 剪贴板（读 / 写）
